...
function isMode(value: unknown): value is ReportMode { return value === "short_essay" || value === "casual_post" || value === "opinion_post" || value === "post"; }
function isGeneratableMode(value: unknown): value is Exclude<ReportMode, "post"> { return value === "short_essay" || value === "casual_post" || value === "opinion_post"; }
...
export async function GET(request: Request) {
  ...
  if (!isMode(mode)) return Response.json({ error: { code: "INVALID_MODE", message: "mode must be short_essay, casual_post, opinion_post, or post" } }, { status: 400 });
...
export async function POST(request: Request) {
  ...
  if (!isLocale(locale)) return Response.json({ error: { code: "INVALID_LOCALE", message: "locale must be zh-CN or en" } }, { status: 400 });
  if (!isGeneratableMode(mode)) return Response.json({ error: { code: "INVALID_MODE", message: "mode must be short_essay, casual_post, or opinion_post" } }, { status: 400 });
