...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (localThoughts.length) { const result = await admin.from("thoughts").upsert(localThoughts.map((item) => ({ id:item.id, owner_id:identity.id, transcript:item.transcript, source:item.source, language:item.language, captured_at:item.capturedAt, captured_day:item.capturedDay, updated_at:item.updatedAt, summary:item.summary, tags:item.tags, personal_tags:item.personalTags, report_included:item.reportIncluded, analysis_status:item.analysisStatus, deleted_at:item.deletedAt })), { onConflict:"id" }); if (result.error) throw new Error(result.error.message); }
    if (localReports.length) { const result = await admin.from("daily_reports").upsert(localReports.map((item) => ({ id:item.id, owner_id:identity.id, date:item.date, locale:item.locale, mode:item.mode, markdown:item.markdown, theme:item.theme, narrative:item.narrative, insights:item.insights, evidence:item.evidence, source_thought_count:item.sourceThoughtCount, generated_at:item.generatedAt })), { onConflict:"id" }); if (result.error) throw new Error(result.error.message); }
