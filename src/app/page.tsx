...
type ReportMode = "short_essay" | "casual_post" | "opinion_post" | "post";
type GeneratableReportMode = Exclude<ReportMode, "post">;
type Report = {
  id: string;
  date: string;
  locale: Locale;
...
function reportModeLabel(mode: ReportMode, locale: Locale) {
  const labels = locale === "zh-CN"
    ? { casual_post: "随手写", opinion_post: "观点帖", short_essay: "日记", post: "旧版推文" }
    : { casual_post: "Casual note", opinion_post: "Opinion post", short_essay: "Diary", post: "Legacy post" };
  return labels[mode];
}
...
export default function Home() {
  ...
  const [reportMode, setReportMode] = useState<GeneratableReportMode>("short_essay");
  ...
  const generateReport = async (outputLocale = reportLocale, outputMode = reportMode) => {
    setNotice(t.loading);
    const response = await fetch("/api/reports/daily", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ date: selectedReportDate, locale: outputLocale, mode: outputMode }),
    });
  ...
  const regenerateReport = async (report: Report) => {
    if (report.mode === "post") return;
    setReportLocale(report.locale);
    setReportMode(report.mode);
    await generateReport(report.locale, report.mode);
  };
        ...
        {section === "settings" ? <section className="desktop-settings">
          ...
          <div className="report-workspace">
            <aside className="report-sidebar">
              <details className="report-source-list"><summary><div className="report-source-heading"><p className="report-label">{locale === "zh-CN" ? "日报素材" : "Source thoughts"}</p><span>{dayThoughts.filter((thought) => thought.reportIncluded).length}/{dayThoughts.length}</span></div></summary>{dayThoughts.length ? <div className="report-source-items">{dayThoughts.map((thought) => <label key={thought.id}><input type="checkbox" checked={thought.reportIncluded} onChange={(event) => void updateThoughtOrganization(thought.id, { reportIncluded: event.target.checked })} /><span><time>{new Intl.DateTimeFormat(locale, { hour: "numeric", minute: "2-digit" }).format(new Date(thought.capturedAt))}</time>{thought.transcript}</span></label>)}</div> : <p className="report-source-empty">{locale === "zh-CN" ? "这一天还没有可选想法。" : "No thoughts are available for this date."}</p>}</details>
              <section className="report-generator">
                <div><p className="report-label">{locale === "zh-CN" ? "输出语言" : "Output language"}</p><div className="report-mode-switch"><button className={reportLocale === "zh-CN" ? "active" : ""} onClick={() => setReportLocale("zh-CN")}>中文</button><button className={reportLocale === "en" ? "active" : ""} onClick={() => setReportLocale("en")}>English</button></div></div>
                <div><p className="report-label">{locale === "zh-CN" ? "写作方式" : "Writing mode"}</p><div className="report-mode-switch"><button className={reportMode === "casual_post" ? "active" : ""} onClick={() => setReportMode("casual_post")}>{locale === "zh-CN" ? "随手写" : "Casual note"}</button><button className={reportMode === "opinion_post" ? "active" : ""} onClick={() => setReportMode("opinion_post")}>{locale === "zh-CN" ? "观点帖" : "Opinion post"}</button><button className={reportMode === "short_essay" ? "active" : ""} onClick={() => setReportMode("short_essay")}>{locale === "zh-CN" ? "日记" : "Diary"}</button></div></div>
                <button className="report-regenerate" onClick={() => void generateReport()}>{locale === "zh-CN" ? "生成日报" : "Generate daily note"}</button>
              </section>
            </aside>
            <section className="report-reading">{reports.length === 0 ? <section className="empty-state report-center-empty"><Image src="/assets/thought-space-orb-v2.png" alt="" width={76} height={76} /><p>{locale === "zh-CN" ? "这一天还没有生成日报。" : "No daily note has been generated for this date."}</p></section> : <div className="report-card-list">{reports.map((item) => { const expanded = expandedReportId === item.id; return <article className={`report-entry ${expanded ? "is-expanded" : ""}`} key={item.id}><header><button className="report-collapse" type="button" onClick={() => setExpandedReportId(expanded ? null : item.id)} aria-expanded={expanded}><span>{expanded ? "▾" : "▸"}</span><div><span className="report-language">{item.locale === "zh-CN" ? "中文" : "English"}</span><span className="report-language">{reportModeLabel(item.mode, locale)}</span><small>{new Intl.DateTimeFormat(locale, { hour: "numeric", minute: "2-digit" }).format(new Date(item.generatedAt))}</small></div><p>{item.narrative}</p></button><span>{item.mode !== "post" && <button className="report-regenerate" onClick={() => void regenerateReport(item)}>{locale === "zh-CN" ? "重新生成" : "Regenerate"}</button>}<button className="report-delete" type="button" onClick={() => setReportPendingDelete(item)}>{locale === "zh-CN" ? "删除" : "Delete"}</button></span></header>{expanded && <><article className="report-markdown">{item.markdown.split("\n").map((line, index) => line.startsWith("## ") ? <h4 key={index}>{line.slice(3)}</h4> : line.startsWith("# ") ? null : line ? <p key={index}>{line}</p> : null)}</article><details className="report-evidence"><summary>{locale === "zh-CN" ? "查看依据" : "View sources"}</summary><ol>{item.evidence.map((source) => <li key={source.thoughtId}><time>{new Intl.DateTimeFormat(locale, { hour: "numeric", minute: "2-digit" }).format(new Date(source.capturedAt))}</time><span>{source.transcript}</span></li>)}</ol></details></>}</article>; })}</div>}</section>
          </div>
